...
[RFC-7677](https://tools.ietf.org/html/rfc7677), and
...
### Client side with channel binding (SCRAM-PLUS)
    ...
    func main() {
        ...
        tlsConn, err := tls.Dial("tcp", "server:port", &tls.Config{})
        if err != nil {
            panic(err)
        }
        defer tlsConn.Close()
        ...
        // Extract channel binding data from TLS connection (TLS 1.3 example)
        cbData, err := tlsConn.ExportKeyingMaterial("EXPORTER-Channel-Binding", nil, 32)
        if err != nil {
            panic(err)
        }

        // Configure channel binding for SCRAM-SHA-256-PLUS
